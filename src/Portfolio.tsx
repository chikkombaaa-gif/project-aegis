    ...
    <header className={`fixed left-0 right-0 top-0 z-40 transition-all ${scrolled ? "border-b border-[oklch(0.55_0.25_260/0.12)] backdrop-blur-xl" : ""}`}>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {links.map((l) => (
            <a key={l.id} href={`#${l.id}`} className="group relative text-xs uppercase tracking-[0.25em] text-[oklch(0.8_0.02_260)] transition hover:text-white">
              {l.label}
        ...
        <Magnetic>
          <a href={`mailto:${PROFILE.email}?subject=Opportunity%20for%20Barath%20V`} className="glass rounded-full px-4 py-2 text-xs uppercase tracking-[0.25em] transition hover:glow-ring">
            Hire me
          </a>
        </Magnetic>
      </div>
...
function Portrait() {
  ...
  const sweepX = useSpring(useTransform(mx, [0, 1], ["-20%", "120%"]), { stiffness: 80, damping: 20 });
  ...
  return (
    <motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} className="relative" style={{ perspective: 1200 }}>
      <div
      ...
      <motion.div className="glass relative overflow-hidden rounded-[1.75rem] p-2" style={{ boxShadow: "var(--shadow-elevated)", rotateX: rx, rotateY: ry, transformStyle: "preserve-3d" }}>
        <motion.div className="relative overflow-hidden rounded-[1.4rem]" animate={{ y: [0, -6, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}>
          <img
            src={PORTRAIT_URL}
          ...
          <motion.div
            ...
            style={{
              background: "linear-gradient(115deg, transparent 40%, oklch(0.95 0.05 260 / 0.18) 50%, transparent 60%)",
              x: sweepX,
              mixBlendMode: "screen",
            }}
          />
          ...
          <div className="absolute inset-x-4 bottom-4 flex items-end justify-between">
            <div>
              <div className="text-[9px] uppercase tracking-[0.35em] text-[oklch(0.75_0.05_260)]">AI & ML Engineer</div>
            ...
            <div className="flex flex-col items-end gap-1 text-right">
              ...
              <span className="text-[9px] uppercase tracking-[0.3em] text-[oklch(0.7_0.01_260)]">Chennai, IN</span>
            </div>
          </div>
        </motion.div>
      </motion.div>
...
function Hero() {
  ...
  return (
    <section id="top" ref={heroRef} className="relative flex min-h-screen items-center overflow-hidden px-6 pt-32 md:pt-24">
      <motion.div style={{ y: heroY, opacity: heroOpacity, filter: heroBlur }} className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-12 md:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.16, 1, 0.3, 1], delay: 0.25 }}>
          ...
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.95, duration: 0.8 }} className="mt-4 max-w-lg text-xl font-normal text-[oklch(0.88_0.02_260)]">
            {PROFILE.role}
          </motion.p>
          <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }} className="mt-5 max-w-xl text-sm leading-relaxed text-[oklch(0.72_0.03_260)] md:text-[15px]">
            {PROFILE.tagline}
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.25, duration: 0.7 }} className="mt-10 flex flex-wrap gap-3">
            <Magnetic>
              <a href="#projects" className="group inline-flex items-center gap-2 rounded-full bg-white px-6 py-3.5 text-sm font-semibold text-[oklch(0.15_0.005_260)] transition hover:bg-[oklch(0.92_0.005_260)]">
                See my work <ArrowUpRight className="h-4 w-4 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
              </a>
            </Magnetic>
            <Magnetic>
              <a href={`mailto:${PROFILE.email}?subject=Opportunity%20for%20Barath%20V`} className="inline-flex items-center gap-2 rounded-full border border-[oklch(0.7_0.01_260/0.25)] px-6 py-3.5 text-sm font-medium text-white transition hover:border-[oklch(0.7_0.01_260/0.45)] hover:bg-[oklch(0.2_0.03_260/0.4)]">
                Contact <Mail className="h-4 w-4" />
              </a>
            </Magnetic>
            <Magnetic>
              <a href={PROFILE.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[oklch(0.7_0.01_260/0.25)] px-5 py-3.5 text-sm font-medium text-[oklch(0.85_0.01_260)] transition hover:border-[oklch(0.7_0.01_260/0.45)]">
                <Github className="h-4 w-4" /> GitHub
              </a>
            </Magnetic>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.45, duration: 0.8 }} className="mt-12 flex flex-wrap gap-x-8 gap-y-3 text-xs uppercase tracking-[0.25em] text-[oklch(0.7_0.03_260)]">
            <span className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5" /> {PROFILE.location}</span>
            <span className="flex items-center gap-2"><GraduationCap className="h-3.5 w-3.5" /> CGPA {PROFILE.cgpa}</span>
            <span className="flex items-center gap-2"><Target className="h-3.5 w-3.5 text-[oklch(0.72_0.09_250)]" /> Pre-final year</span>
          </motion.div>
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, delay: 0.45, ease: [0.16, 1, 0.3, 1] }} className="flex justify-center">
          <Portrait />
        </motion.div>
      </motion.div>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.65, duration: 0.8 }} style={{ opacity: scrollCueOpacity }} className="pointer-events-none absolute bottom-8 left-1/2 z-10 -translate-x-1/2">
        <div className="flex flex-col items-center gap-2">
          <span className="text-[9px] uppercase tracking-[0.4em] text-[oklch(0.7_0.03_260)]">Scroll</span>
          <div className="relative h-9 w-[1px] overflow-hidden bg-[oklch(0.55_0.25_260/0.25)]">
            <motion.div className="absolute inset-x-0 top-0 h-3" style={{ background: "linear-gradient(180deg, transparent, oklch(0.75 0.22 260))" }} animate={{ y: ["-100%", "300%"] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }} />
          </div>
        </div>
      </motion.div>
    </section>
  ...
  return (
    <section id={id} className="relative px-6 py-28 md:py-32">
      <div className="mx-auto max-w-6xl">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="mb-14">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="glass col-span-1 rounded-3xl p-8 md:col-span-2">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {VALUE.map((s, i) => (
              <motion.div key={s.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="rounded-2xl border border-[oklch(0.55_0.25_260/0.2)] bg-[oklch(0.15_0.05_260/0.4)] p-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.12 }} className="glass rounded-3xl p-8">
    ...
    <Section id="skills" eyebrow="Capabilities" title="Tools I use to deliver.">
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        {SKILLS.map((s, i) => {
          const Icon = s.icon;
          return (
            <motion.div key={s.group} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, delay: i * 0.06 }} whileHover={{ y: -6 }} className="glass group relative overflow-hidden rounded-3xl p-6">
              <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full opacity-0 blur-2xl transition group-hover:opacity-60" style={{ background: "oklch(0.55 0.25 260 / 0.6)" }} />
              <div className="relative flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-[10px] uppercase tracking-[0.3em] text-[oklch(0.7_0.03_260)]">Domain</div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {s.items.map((it) => (
                  <span key={it} className="rounded-full border border-[oklch(0.7_0.05_260/0.25)] bg-[oklch(0.2_0.05_260/0.6)] px-3 py-1 text-[11px] text-[oklch(0.85_0.02_260)]">{it}</span>
                ))}
              </div>
              <div className="mt-5 h-[3px] w-full overflow-hidden rounded-full bg-[oklch(0.22_0.005_260)]">
                <motion.div initial={{ width: 0 }} whileInView={{ width: `${s.level}%` }} viewport={{ once: true }} transition={{ duration: 1.15, ease: "easeOut", delay: 0.15 }} className="h-full" style={{ background: "oklch(0.72 0.09 250)" }} />
              </div>
            </motion.div>
          );
        })}
  ...
  return (
    <Section id="projects" eyebrow="Selected Work" title="Evidence of how I work.">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {PROJECTS.map((p, i) => (
          <motion.article key={p.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.7, delay: i * 0.1 }} whileHover={{ y: -8 }} className="glass group relative flex flex-col overflow-hidden rounded-3xl p-7">
            <div className="pointer-events-none absolute inset-0 opacity-0 transition duration-500 group-hover:opacity-100" style={{ background: "radial-gradient(circle at 30% 15%, oklch(0.55 0.25 260 / 0.28), transparent 55%)" }} />
            <div className="relative flex flex-1 flex-col">
              <div className="mb-5 flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-1.5">
                  ...
                  <span className="text-[10px] tracking-[0.15em] text-[oklch(0.6_0.03_260)]">{p.year}</span>
                </div>
              </div>
              <h3 className="display text-xl leading-snug text-white">{p.title}</h3>
              <p className="mt-3 flex-1 text-sm leading-relaxed text-[oklch(0.75_0.03_260)]">{p.body}</p>
              <div className="mt-5 border-t border-[oklch(0.55_0.25_260/0.15)] pt-4 text-[10px] uppercase tracking-[0.18em] text-[oklch(0.72_0.1_260)]">{p.outcome}</div>
              <div className="mt-4 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span key={t} className="rounded-full border border-[oklch(0.55_0.25_260/0.3)] px-2.5 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[oklch(0.8_0.1_260)]">{t}</span>
                ))}
              </div>
            </div>
          </motion.article>
      ...
      <motion.a href={PROFILE.github} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass group mt-8 flex items-center justify-between rounded-3xl p-6 transition hover:glow-ring">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-xs text-[oklch(0.7_0.03_260)]">Code, experiments, and implementation details</div>
          </div>
        </div>
...
function Contact() {
  const items = [
    { icon: Mail, label: "Email", value: PROFILE.email, href: `mailto:${PROFILE.email}?subject=Opportunity%20for%20Barath%20V` },
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="glass rounded-3xl p-8">
          ...
          <div className="mt-8 space-y-3">
            {items.map((it) => {
              const Icon = it.icon;
              return (
                <a key={it.label} href={it.href} target={it.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="group flex items-center justify-between rounded-2xl border border-[oklch(0.55_0.25_260/0.2)] bg-[oklch(0.15_0.05_260/0.4)] p-4 transition hover:border-[oklch(0.55_0.25_260/0.55)] hover:bg-[oklch(0.55_0.25_260/0.08)]">
                  <div className="flex items-center gap-4">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[oklch(0.55_0.25_260/0.15)] text-[oklch(0.8_0.15_260)]">
                      <Icon className="h-4 w-4" />
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.25em] text-[oklch(0.7_0.03_260)]">{it.label}</div>
        ...
        <motion.form
          ...
          onSubmit={(e) => {
            ...
            window.location.href = `mailto:${PROFILE.email}?subject=Opportunity%20from%20${encodeURIComponent(name)}&body=${encodeURIComponent(msg)}`;
          }}
          className="glass rounded-3xl p-8"
        >
          <div className="mb-6 text-xs uppercase tracking-[0.3em] text-[oklch(0.7_0.15_260)]">Direct message</div>
          <div className="space-y-4">
            {[
              ...
              <label key={f.name} className="block">
                <span className="text-[10px] uppercase tracking-[0.25em] text-[oklch(0.7_0.03_260)]">{f.label}</span>
                <input required name={f.name} type={f.type} className="mt-1.5 w-full rounded-xl border border-[oklch(0.55_0.25_260/0.25)] bg-[oklch(0.12_0.04_260/0.6)] px-4 py-3 text-sm text-white outline-none transition focus:border-[oklch(0.55_0.25_260)] focus:glow-ring" />
              </label>
            ))}
            <label className="block">
              <span className="text-[10px] uppercase tracking-[0.25em] text-[oklch(0.7_0.03_260)]">Message</span>
              <textarea required name="message" rows={4} placeholder="Role, team, or what you're looking for…" className="mt-1.5 w-full resize-none rounded-xl border border-[oklch(0.55_0.25_260/0.25)] bg-[oklch(0.12_0.04_260/0.6)] px-4 py-3 text-sm text-white outline-none transition placeholder:text-[oklch(0.5_0.02_260)] focus:border-[oklch(0.55_0.25_260)] focus:glow-ring" />
            </label>
            <button type="submit" className="group flex w-full items-center justify-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-[oklch(0.15_0.005_260)] transition hover:bg-[oklch(0.92_0.005_260)]">
...
function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-[oklch(0.55_0.25_260/0.15)] px-6 pb-12 pt-24">
      <motion.div aria-hidden initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }} className="pointer-events-none mx-auto mb-16 max-w-[1600px] px-4 text-center">
        <div
          className="display text-[11vw] leading-[0.9] tracking-tight md:text-[9vw]"
          style={{
            background: "linear-gradient(180deg, oklch(0.9 0.03 260 / 0.2), transparent 90%)",
      ...
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 md:flex-row">
        <div className="flex items-center gap-3">
          <span className="flex h-7 w-7 items-center justify-center rounded-md border border-[oklch(0.75_0.05_260/0.25)] text-[10px] font-semibold tracking-widest text-white/85">BV</span>
          <span className="text-xs tracking-[0.2em] text-[oklch(0.75_0.02_260)]">© {new Date().getFullYear()} {PROFILE.fullName}</span>
        </div>
        <div className="hidden text-[10px] uppercase tracking-[0.35em] text-[oklch(0.6_0.03_260)] md:block">Built for impact · Chennai, IN</div>
        <a href="#top" className="glass rounded-full px-4 py-2 text-[10px] uppercase tracking-[0.3em] text-white transition hover:glow-ring">Back to top ↑</a>
      </div>
    </footer>
  );
}
